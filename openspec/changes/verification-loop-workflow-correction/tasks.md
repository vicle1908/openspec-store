...
- [ ] 3.1 Sweep all OpenSpec-related user-local references for stale unscoped validation, masked OpenSpec verifier pipelines, and status/implementation conflation; classify each remaining match as corrected, historical, or requiring a follow-up patch.
- [ ] 3.2 Validate this change with `openspec validate verification-loop-workflow-correction --strict --store openspec-store`.
- [ ] 3.3 Run `openspec validate --all --strict --no-interactive --store openspec-store` and classify any unrelated result.
- [ ] 3.4 Re-read the corrected guidance and confirm the exact state model, bounded retry rule, and safe task-counting recipe are present.
- [ ] 3.5 Commit only this change directory in the shared store after explicit closure authorization.
- [ ] 3.6 Archive this change only after tasks 3.1–3.5 are evidenced and checked; verify active absence, archive presence, all archived tasks checked, and post-archive full validation.
