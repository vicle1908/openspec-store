...
- [ ] 2.1 Add failing tests for dynamic `TDT_HOME` resolution, unset/empty fallback, tilde expansion, and no import-time snapshots.
...
- [ ] 2.5 Seed canary secrets and assert they never occur in stdout, stderr, JSON output, logs, exceptions, or migration manifests.
...
- [ ] 3.2 Implement explicit environment profiles and source-provenance reporting while preserving the four existing loader scenarios.
...
- [ ] 3.4 Implement `tdt config doctor` text/JSON/strict modes and the versioned cross-repo conformance manifest.
...
- [ ] 4.4 Build/install provider and consumer wheels in a clean temporary environment and verify imports and smoke commands without editable sibling paths.

## 5. Supporting TDT consumers

- [ ] 5.1 In `tdt-observability`, declare the provider dependency, migrate PID/config/database/log-source/log paths, and test alternate roots plus no import-time snapshots.
- [ ] 5.2 In `tdt-sheets`, declare the provider dependency, resolve the fallback credential through `credentials/` with the legacy compatibility link, and test missing/broken/valid link behavior without exposing credential content.
...
- [ ] 6.2 Add tests that every conformance-manifest repository exists, every exception has owner/reason/expiry, and expired exceptions fail.
...
- [ ] 9.4 Update Graphify indexes where present, commit each owning repository independently, and record provider-first rollout and consumer-first rollback order.
