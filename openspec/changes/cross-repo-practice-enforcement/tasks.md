...
- [x] 1.1 Create `~/Developer/workspace-python-template/` with:
...
- [x] 1.2 Verify template passes all gates: `uv run ruff check .`, `uv run ruff format --check .`, `uv run mypy .`
- [x] 1.3 Commit template

## Phase 2: Standardize Tool Versions (4 tasks)

- [x] 2.1 Update ruff version floor to `>=0.16.1` in all 16 repos' `[dependency-groups]`
  - Run `uv lock` in each repo to regenerate lockfiles
  - Verify `uv run ruff check .` passes in each repo
- [x] 2.2 Update mypy version floor to `>=2.3.0` in all 16 repos' `[dependency-groups]`
  - Run `uv lock` in each repo
  - Verify `uv run mypy .` passes (or add overrides for third-party gaps)
- [x] 2.3 Update pytest version floor to `>=9.1.1` (and pytest-asyncio>=1.4.0) in all 16 repos
  - Run `uv lock` in each repo
  - Verify `uv run pytest -q` passes
  - **tdt-observability**: clean up duplicate pytest entries in dependency-groups
- [x] 2.4 Run `uv run --frozen` in all repos to verify lockfile sync

## Phase 3: Standardize Ruff Config (3 tasks)

- [x] 3.1 Update ruff lint select rules to canonical 19-rule set in repos with drift:
...
- [x] 3.2 Add canonical per-file-ignores to all repos:
  - `"__init__.py" = ["F401"]`
  - `"tests/**/*.py" = ["F841", "B007", "E402", "S101", "S108", "ARG001", "ARG002", "SLF001"]`
  - Keep existing repo-specific overrides (document in template README)
- [x] 3.3 Run `uv run ruff check . --fix` then `uv run ruff check .` in all 16 repos
...
- [x] 4.1 Create `.pre-commit-config.yaml` from canonical template in: ai-harness-skills, code-daily-scan (add shellcheck for bash scripts), tdt-observability, tdt-sheets
- [x] 4.2 Install and verify: `uv run pre-commit install && uv run pre-commit run --all-files`

## Phase 5: Standardize Existing Pre-Commit Configs (3 tasks)

- [x] 5.1 Update Pattern A repos (agent-core, docs-sync, harness) — update revs to canonical:
...
- [x] 5.2 Update Pattern B repos (ai-review, tdt-core, webhook-receiver) — add local mypy/pytest hooks:
...
- [x] 5.3 Update Pattern C repos (jira-\*, browser-cli, ops-auto) — update revs:
...
- [x] 6.1 Audit and fix missing version pins across ALL cross-repo dependencies:
...
- [x] 6.2 Verify `uv run --frozen` works in all repos (lockfile sync check)

## Phase 7: Validation and Documentation (3 tasks)

- [x] 7.1 Run `check-enforcement.sh` across all 16 repos — verify zero drift
- [x] 7.2 Update workspace documentation (AGENTS.md if needed)
- [x] 7.3 Commit all changes per-repo, then archive openspec change:
  ```
