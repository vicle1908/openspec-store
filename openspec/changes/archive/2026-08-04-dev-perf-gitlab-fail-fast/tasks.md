...
- [ ] 1. Add `validate_gitlab_auth` helper to
...
- [ ] 2. Add `joined_via_none` counter to `JoinMethod` tally in
...
- [ ] 3. Add `_gitlab_required_mode()` helper that reads
  `DEV_PERFORMANCE_GITLAB_REQUIRED` env var (default False) and
  passes result to `_validate_gitlab_auth` in `cli.py`.
- [ ] 4. Document new env var in
  `jira-daily-reports/config/env-sample.env` and
  `tdt-meta/docs/workflows/dev-performance-runbook.md` (new doc).

## Tests

- [ ] 5. Add `tests/dev_performance/test_source_validation.py` with
  5 unit tests covering: success, 401 warn, 401 raise, empty user
  attribute, summary counter.

## Validation

- [ ] 6. Run `ruff check . --fix && ruff format .` in
  `jira-daily-reports/`.
- [ ] 7. Run `mypy jira-daily-reports --strict` in tdt-meta.
- [ ] 8. Run `pytest -x` in `jira-daily-reports/tests/dev_performance/`.
- [ ] 9. Run `openspec validate --strict dev-perf-gitlab-fail-fast`.
- [ ] 10. Live dry-run: `cd jira-daily-reports && uv run dev-performance
  --dry-run` — confirm `joined_via_none` key appears in summary.

## Documentation

- [ ] 11. Create `tdt-meta/docs/workflows/dev-performance-runbook.md` with
  "GitLab authentication" section explaining the new env knob and the
  fail-fast ERROR line operators should grep for.
- [ ] 12. Cross-link the new runbook from
  `tdt-meta/.agents/modules/coding.md` and
  `jira-daily-reports/README.md` if present.

## Cleanup

- [ ] 13. Commit the changes with body referencing
  `Part-of: dev-perf-gitlab-fail-fast`.
- [ ] 14. Once all tasks check out, run `opsx:archive` to move
...
- [ ] 15. Add `jira_daily_reports/_env_quoting.py` with
...
- [ ] 16. Update `jira-daily-reports/src/jira_daily_reports/dev_performance/sheet_writer.py`
  `persist_gid_to_sheet_links` to call `format_sheet_links_line()` on the
  final entries.
- [ ] 17. Update `jira-daily-reports/src/jira_daily_reports/catalog/writer.py`
  `Writer._persist_gid_to_sheet_links` to call `format_sheet_links_line()`.
- [ ] 18. Add `jira-daily-reports/tests/test_env_quoting.py` covering:
  unquoted parse, quoted parse, format-with-multiple-entries, format+parse
  round-trip, idempotent re-format.
- [ ] 19. Run `ruff check . --fix && ruff format .` and `pytest -x
  tests/test_env_quoting.py` in `jira-daily-reports/`.
- [ ] 20. Manually verify: `bash -c 'set -a; source ~/.tdt/.env; set +a;
...
- [ ] 21. Update `jira-daily-reports/src/jira_daily_reports/logging_config.py`
...
- [ ] 22. Add `jira-daily-reports/tests/test_logging_config.py` covering:
  propagation is True after import, WARNING records from a child logger
  reach the root logger, double-call is idempotent.
- [ ] 23. Update `jira-daily-reports/tests/catalog/test_joiner.py`
  ...
  not time-sensitive.
- [ ] 24. Run the full combined suite in the order pytest picks by default:
  ...
  Confirm 0 failures.
- [ ] 25. Run `ruff check . --fix && ruff format .` and `mypy
  jira-daily-reports --strict` on the changed files.
- [ ] 26. Run `openspec validate --strict dev-perf-gitlab-fail-fast`.
