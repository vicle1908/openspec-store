...
- [x] 4.5 Commit adapter changes and OpenSpec store changes separately with scoped messages.
  - Evidence: adapter repository is clean with implemented changes committed across the adapter history (functional commits `82a0149`, `3bf7b35`, plus LaunchAgent commits); OpenSpec change was committed at `402219e`.
- [x] 4.6 Archive only after all three live provider gates are green and the adapter effort field is independently observed.
  - Evidence: archived via `openspec archive`. All three live gates green; review APPROVE_WITH_BLOCKER with no functional defect; CI guard accepted as out-of-scope follow-up.
