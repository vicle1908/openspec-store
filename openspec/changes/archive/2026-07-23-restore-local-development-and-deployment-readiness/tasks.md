...
- [x] 2.11 Prove clean full-stack startup and smoke success twice without duplicating namespaces, topics, connectors, migrations, or externally visible workflow effects, and retain both evidence manifests. Evidence: `artifacts/verification/local/compose-acceptance/compose-verify-c/manifest.json` and `artifacts/verification/local/compose-acceptance/compose-verify-d/manifest.json`.
- [x] 2.12 Replace the notification service's process-local repository with durable PostgreSQL persistence shared by API and worker roles, and prove dispatch state survives the cross-process workflow boundary.
- [x] 2.13 Replace payment, inventory, and shipping `/metrics` stubs with real telemetry handlers and prove required metrics reach the pinned Collector pipeline.
- [x] 2.14 Rebuild the canonical service images from the current source and prove every running Temporal worker uses its SDK-valid dash-separated Deployment name independently from its stable dotted task queue.
- [x] 2.15 Add an inventory-owned stock-level command and API, provision the smoke product through that deployed boundary, and keep inventory validation read-only before the reservation mutation.
- [x] 2.16 Reconcile and prove the customer GDPR export contract: asynchronous POST plus durable GET, complete Article 15 payload, stable content hash and export identity on replay, no duplicate workflow start, and fail-closed cross-tenant authorization. Evidence: `make -C services/customer-service test-unit`; focused fail-closed ownership tests in `internal/adapters/http` and `internal/config`; live `gdpr_export` stage in `artifacts/kind/go-go-microservices-local-20260723/final3-local-acceptance-dirty.36029e4a5b62-20260723105425/smoke-report.json`.
...
- [x] 3.3 Build uniquely tagged images for all eight services, verify their target architectures, and load them into kind or publish them to the documented local registry. Evidence: `make kind-up`; `artifacts/kind/go-go-microservices-local-20260723/final3-local-acceptance-dirty.36029e4a5b62-20260723105425/images.tsv`, `node-images.txt`, and per-image node inspection JSON.
...
- [x] 3.5 Add local aggregation and ordered application of prerequisites plus all eight service overlays, then wait a bounded time for workloads and Temporal workers to converge. Evidence: clean `make kind-up`; `artifacts/kind/go-go-microservices-local-20260723/final3-local-acceptance-dirty.36029e4a5b62-20260723105425/rollout-status.json`, `debezium-order-source-connector.json`, `temporal-task-queues.tsv`, and `temporal-worker-readiness.json`.
- [x] 3.6 Run the cross-service Kubernetes smoke and telemetry assertions against kind and retain the tested image identifiers, rendered manifests, rollout status, and smoke report. Evidence: clean `make kind-up`; `artifacts/kind/go-go-microservices-local-20260723/final3-local-acceptance-dirty.36029e4a5b62-20260723105425/smoke-report.json`, `rendered/`, `running-images.tsv`, and `diagnostics/`.
...
- [x] 4.6 Add pinned Kubernetes schema and policy validation plus server-side dry-run for every rendered overlay, with tests for invalid quantities, APIs, selectors, and security settings. Evidence: `artifacts/deployment-validation/20260723T093015Z-679f5b6f95/manifest.json`.
...
- [x] 7.1 Audit current OpenSpec requirements and task checkboxes, downgrade unsupported implementation claims, and link every retained readiness claim to its acceptance command and evidence manifest. Local readiness evidence is enumerated below; staging, production, multi-architecture publishing, CI enforcement, promotion, and rollback claims remain unchecked.
...
- [x] 8.1 Run the complete Compose acceptance twice from a clean unique project and retain passing manifests proving all eight services, Temporal workers, reporting, and required telemetry. Evidence: `artifacts/verification/local/compose-acceptance/compose-verify-c/manifest.json` and `artifacts/verification/local/compose-acceptance/compose-verify-d/manifest.json`.

## Local readiness evidence audit

| Claim | Acceptance command | Retained evidence |
|---|---|---|
| Complete Compose lifecycle is repeatable | `make dev-up && make dev-smoke` in two clean unique projects | `artifacts/verification/local/compose-acceptance/compose-verify-c/manifest.json`; `compose-verify-d/manifest.json` |
| Customer GDPR export is durable, idempotent, and ownership checks fail closed | `make -C services/customer-service test-unit`; clean `make kind-up` | focused customer-service tests; final kind `smoke-report.json` |
| Local kind images match the final source tree and host architecture | clean `make kind-up` on Darwin/arm64 | final kind `images.tsv`, `node-images.txt`, per-image inspection JSON, and `running-images.tsv` |
| Local prerequisites, migrations, CDC, services, and Temporal workers converge | clean `make kind-up` | final kind `rollout-status.json`, `debezium-order-source-connector.json`, `temporal-task-queues.tsv`, and `temporal-worker-readiness.json` |
| Cross-service behavior and required traces, metrics, and logs pass | clean `make kind-up` | final kind `smoke-report.json` and `smoke-report.log` |
| Failure and live-state diagnostics are retained | `make kind-diagnostics` | final kind `diagnostics/` |
| Repository deployment checks are exhaustive and fail closed | `PATH="$PWD/.tools/bin:$PATH" make validate-deployment` | `artifacts/deployment-validation/20260723T110538Z-40a7c5d6d1/manifest.json`: 186 passed, one host-capacity failure (workspace free space below 30 GiB), one non-blocking legacy-spec warning |
