# Capability: tdt-scheduler-cancel-orphan-enqueued-cli

## Purpose
Provide a one-shot operator CLI that cancels `ENQUEUED` DBOS workflow rows
whose `application_version` is no longer registered in
`dbos.application_versions`, replacing the manual SQL we ran on 2026-06-17.
...
`cancelled`, `current_versions`, `threshold_hours`, `older_than_iso`, and
