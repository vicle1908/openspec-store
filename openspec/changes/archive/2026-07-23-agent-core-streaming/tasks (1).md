...
- [ ] 3.4 Implement `get_stream_writer()` support for custom node emissions
- [ ] 3.5 Implement `stream_mode="checkpoints"` for checkpoint events during streaming
...
- [ ] 4.3 Support local (stdio) and remote (Streamable HTTP, SSE) MCP servers
- [ ] 4.4 Wire MCP tools through ToolRegistry for governance
- [ ] 4.5 Add MCP server configuration to settings
- [ ] 4.6 Support provider-native MCP execution when available (nhà cung cấp dịch vụ AI, nhà cung cấp dịch vụ AI)
...
- [ ] 5.5 Verify provider-adaptive behavior (nhà cung cấp dịch vụ AI, nhà cung cấp dịch vụ AI, Google)
...
- [ ] 7.3 Implement stream pause/resume mechanism for approval workflow
...
- [ ] 9.1 Create `tests/agent_base/test_streaming.py`
- [ ] 9.2 Test streaming yields text chunks incrementally
- [ ] 9.3 Test streaming with structured output yields validated model instances
- [ ] 9.4 Test streaming completion yields `done=True` chunk
- [ ] 9.5 Test approval gate interrupts stream correctly
- [ ] 9.6 Test stream resumes after approval granted
- [ ] 9.7 Test LangGraph streaming mode="messages" yields token chunks
- [ ] 9.8 Test get_stream_writer() custom emissions
- [ ] 9.9 Test backward compatibility: `run()` still works unchanged
- [ ] 9.10 Run `pytest tests/agent_base/ -x`
