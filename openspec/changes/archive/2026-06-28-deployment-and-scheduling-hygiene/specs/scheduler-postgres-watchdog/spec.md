# Capability: scheduler-postgres-watchdog

## Purpose

Closes the **Critical** SPOF that the agent-core Postgres container is the
sole store of DBOS schedule metadata, debouncer queue state, and
`agentmemory` data, with no backup and no healthcheck on the scheduler
