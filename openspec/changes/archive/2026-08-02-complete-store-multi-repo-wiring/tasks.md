...
- [ ] 1.9 Run `openspec update` in each repo to generate skill files
...
- [ ] 2.1 Add remote: `git -C ~/Developer/openspec-store remote add origin <url>` (deferred)
- [ ] 2.2 Add `remote: <url>` to `.openspec-store/store.yaml` (deferred)
- [ ] 2.3 Push: `git -C ~/Developer/openspec-store push -u origin main` (deferred)
- [ ] 2.4 Verify: `openspec store doctor openspec-store --json` shows remote non-null (deferred)
