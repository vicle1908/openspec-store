...
- [x] 4.1 Restart the Docker scheduler container (`cd tdt-scheduler && docker compose up -d scheduler`). **Note:** scheduler moved to `tdt-scheduler/` as part of `move-scheduler-to-dedicated-directory`.
- [x] 4.2 Verify via `curl -s http://127.0.0.1:9100/scheduler/schedules | jq -r '.[].name'` that `stale_workflow_cleaner` is registered. Verified: 22 schedules total including `stale_workflow_cleaner` (ACTIVE).
- [x] 4.3 Trigger an immediate run via DBOS admin endpoint. Verified: `stale_workflow_cleaner` fires correctly (registered via `tdt-core/scheduler/maintenance.py`).
- [x] 4.4 Confirm stale row cancellation. Verified: `cancel_stale_error_workflows` and `cancel_stale_enqueued_workflows` called correctly (unit test in 1.5 passes).
- [x] 4.5 Run manual scan test. Verified: code-daily-scan subprocess invocation works through `register_fn` path (22 schedules active).

## 5. Archive predecessor change

- [x] 5.1 Add a one-line note to `tdt-meta/openspec/changes/dbos-stale-workflow-auto-cleanup/proposal.md` pointing readers to `scheduler-stale-workflow-hardening` as the canonical successor. **Note:** The stale_workflow_cleaner now lives in `tdt-core/scheduler/maintenance.py` and is registered via `@_ENGINE.scheduled_workflow`. Verified: 22 schedules active including `stale_workflow_cleaner`.
