...
The system SHALL provide a `skills doctor` command that validates the active profile and reports issues.
...
- **WHEN** the same skill name exists in multiple directories
- **THEN** doctor reports which version is active (shadow winner) and which are shadowed
...
- **THEN** output is a JSON object with `warnings`, `errors`, and `info` arrays, each containing structured diagnostic objects
