...
No API keys, passwords, tokens, or credentials SHALL appear in source code. All secrets SHALL be loaded from `~/.tdt/.env` via `load_tdt_env()`.

#### Scenario: Credential loading

- **WHEN** code needs credentials
- **THEN** it SHALL use `tdt_core.env.load_tdt_env()` and `os.getenv()`
- **AND** it SHALL NOT read from hardcoded paths or string literals
