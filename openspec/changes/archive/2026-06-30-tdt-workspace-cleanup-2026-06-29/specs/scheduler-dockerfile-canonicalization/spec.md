# Capability: scheduler-dockerfile-canonicalization

## Purpose

Define the single source of truth for the TDT scheduler Docker image. Today, two scheduler Dockerfiles exist in the workspace: a live one (`deployments/scheduler/Dockerfile`, `user=agent`) and an orphan (`agent-core/deployments/scheduler/Dockerfile`, `user=scheduler`) that is never built. This spec pins the canonical location, the canonical user, and the redeploy protocol that ensures no breakage before deleting the now-superseded file.
