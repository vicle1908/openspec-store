# Capability: python-syntax-modernization

## Purpose

Define the contract that all Python source code in TDT ecosystem repositories MUST use Python 3 syntax exclusively, with the modern parenthesized except-tuple form being the only acceptable `except` clause shape. This rules out Python 2 `except <Type1>, <Type2>:` aliases and any other deprecated idiom that ruff `UP`/`C4`/`B`/`SIM` rules catch.
