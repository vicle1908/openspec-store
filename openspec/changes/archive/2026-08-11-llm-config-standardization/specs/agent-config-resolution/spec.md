...
Agent-specific YAML files MUST NOT contain top-level keys other than `model` and `runtime`. Unknown top-level keys SHALL be rejected with a clear error message listing the offending keys.
