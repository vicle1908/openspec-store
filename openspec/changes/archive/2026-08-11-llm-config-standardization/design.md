...
**Choice**: `load_agent_config(agent_name, *, config_path=None)` lives in `tdt_core.config_loader`; the optional path replaces the standard per-agent overlay path for explicit CLI/config-file overrides.
...
- Unknown keys: rejected with ``ConfigError`` (makes misconfig visible)
