...
The workspace SHALL provide a canonical agent-core integration contract document for sibling repos and specialist agents under `$WORKSPACE/tdt-meta/docs/agent-core/`.

#### Scenario: Contract document is present
- **WHEN** a developer looks under `$WORKSPACE/tdt-meta/docs/agent-core/`
...
### Requirement: The agent-core invocation contract is deferred to Phase 2
Phase 1 SHALL NOT define the consumer invocation contract (input schema, result shape, error semantics); it SHALL record that this is a Phase 2 deliverable so adapters are not built against an undefined interface.

#### Scenario: Invocation contract is explicitly out of scope
- **WHEN** a reader looks for the invocation/payload contract in the Phase 1 document
- **THEN** the document states the invocation contract (input/result/error shape) is a Phase 2 deliverable and is intentionally undefined here
