...
The operational contract must work on macOS arm64 and Linux amd64. Docker Compose remains the fastest local integration topology; kind provides production-shape Kubernetes validation. The existing domain, REST, Protobuf, outbox, Kafka, and Temporal workflow contracts remain unchanged. Official behavior used by this design includes Docker Compose interpolation, health-gated dependencies and `up --wait`; Temporal namespace registration for self-hosted services; the current OpenTelemetry Collector internal-telemetry schema; Kustomize overlays and image transforms; kind image loading/local registry workflows; Argo CD Git/cluster generators; and GHCR's `packages: write` permission.
...
- Preserve service ownership, database transaction boundaries, at-least-once event delivery, idempotency, and public contract versions.

**Non-Goals:**

- Change REST endpoints, Protobuf schemas, Kafka event versions, aggregate ownership, or database schemas.
