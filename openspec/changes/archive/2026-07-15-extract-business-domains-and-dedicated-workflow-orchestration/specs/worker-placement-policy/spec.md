...
## Purpose
A platform-wide rule SHALL require that the Temporal worker for a service lives in the same Go module as the service's business logic. The worker SHALL NOT live in a shared "workflow service" module. The worker SHALL be deployed as a separate container from the API and orchestrator roles so the API and worker scale independently. The worker container SHALL share the service's Postgres pool with the API role via the same `DATABASE_URL`. The worker SHALL NOT have a separate database. This is the platform's worker-placement policy; it is a permanent architectural rule, not a temporary arrangement.
