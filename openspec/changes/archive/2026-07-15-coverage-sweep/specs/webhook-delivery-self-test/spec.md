...
Define the contract for the periodic self-test that detects silent ingress failures before
they reach the next MR review window.
...
3. Cross-check the public edge by issuing a HEAD request to the public
   hostname root to confirm the TLS handshake completes within 5s.
4. Record the observation to the on-disk DLQ-adjacent file at
   `~/.tdt/state/webhook-selftest-observations.jsonl` (one JSON object
   per line, capped at 720 lines = 60 hours at 5min cadence).
5. If the last 3 observations all report `primary_status=down`, write
