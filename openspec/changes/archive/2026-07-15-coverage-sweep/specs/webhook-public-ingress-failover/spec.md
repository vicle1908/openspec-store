...
Define the public ingress failover contract for `webhook-receiver`: a Tailscale Funnel URL as
the always-on primary, a ngrok free-tier URL as the documented hot spare, and the on-disk
state file that selects which URL GitLab is currently pointed at.
