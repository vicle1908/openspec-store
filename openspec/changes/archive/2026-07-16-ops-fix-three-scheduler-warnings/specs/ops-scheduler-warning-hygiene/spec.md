...
Define three small contracts that make the scheduler's log signal
cleaner and the env-loading observable:

1. Timezone names that are documented IANA aliases SHALL resolve
   transparently, not emit a warning.
2. The catalog differ SHALL emit its `primary_key_remap` warning at
   most once per `(Kind, Name)` collision per refresh, with the
   full set of colliding field_ids exposed on the structured result.
3. `tdt_core.env.load_tdt_env()` SHALL capture the third-party
   `python-dotenv` "could not parse" warning into a structured
   diagnostic surface so operators can see which env var was lost.
