...
## Why
The `create_model()` function in `agent-core/_ai/models.py` needs to resolve LLM provider endpoints and API keys. The active setup uses giaoduc, while the resolver must remain provider-neutral and configuration-driven.
...
2. **Priority chain**: Explicit env vars > config.yaml > native provider environment
...
| Anthropic Messages | `/v1/messages` | `anthropic:Advance` | ✅ Verified and active |
