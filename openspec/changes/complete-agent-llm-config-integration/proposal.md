...
- `cli-provider-profile-resolution`: distinguish CLI adapter identity from canonical provider ID, distinguish genuine mapping absence from canonical-source or projection failure, and require exactly two durable dependency-bound live consumer rows with comprehensive drift invalidation.
