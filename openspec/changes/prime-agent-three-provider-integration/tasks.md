...
- [ ] 1.6 `APPLY-GO` Obtain explicit operator approval for live apply.
...
- [ ] 2.1 `INSTALL-01` Capture the pre-install touched-surface manifest.
  ...
  - Evidence: `evidence/INSTALL-01/pre-state.json`.
- [ ] 2.2 `INSTALL-02` Verify the retained official installer.
...
- [ ] 2.3 `INSTALL-03` Rehearse pinned installation in an isolated temporary HOME with a minimal environment.
...
- [ ] 2.4 `INSTALL-04` Install or select the approved stable runtime for live apply.
...
- [ ] 2.5 `ISOLATE-01` Create isolated config, session, kernel, and daemon boundaries for acceptance.
...
- [ ] 3.1 `CONFIG-01` Materialize the reviewed credential-free `models.json` template in the isolated agent directory.
...
- [ ] 3.2 `CONFIG-02` Verify model registry loading.
...
- [ ] 3.3 `SECRET-01` Verify credential isolation before provider calls.
...
- [ ] 4.1 `WIRE-RESP-01` Observe standard Responses metadata for `shopapikey/fable-5` through Prime Agent.
...
- [ ] 4.2 `WIRE-RESP-02` Observe standard Responses metadata for all retained cockpit models.
  - Same pass/failure rules as `WIRE-RESP-01`.
  - Evidence: `evidence/WIRE-RESP-02/`.
- [ ] 4.3 `WIRE-MSG-01` Verify Giaoduc Messages path and authentication.
...
- [ ] 5.1 `INFER-01` Run isolated no-tool exact-sentinel probes for every retained alias.
...
- [ ] 5.2 `STREAM-01` Verify text, terminal, reasoning, and usage events.
...
- [ ] 5.3 `TOOL-01` Verify tool-call argument reconstruction and tool-result replay in a disposable worktree.
...
- [ ] 5.4 `ERROR-01` Verify invalid auth, unsupported model, timeout, rate limit, malformed stream, interruption, and context overflow.
...
- [ ] 5.5 `HANDOFF-01` Verify at least one cross-provider session handoff.
...
- [ ] 6.1 `WORKSPACE-01` Inspect and verify context/resources in a trusted disposable worktree.
...
- [ ] 6.2 `MCP-01` Verify only the existing mcp-router aggregate route if supported.
...
- [ ] 6.3 `ACP-01` Verify ACP from installed help/source if supported.
...
- [ ] 7.1 `ROLLBACK-01` Rehearse complete install/config/rollback in isolated HOME.
...
- [ ] 7.2 `LIVE-01` Apply to real user state only after `APPLY-GO` and isolated gates pass.
...
- [ ] 7.3 `REAPPLY-01` Verify rollback and reapply if Prime Agent is to remain installed.
