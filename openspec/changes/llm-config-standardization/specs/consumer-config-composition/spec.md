...
Defines the composable configuration pattern for agent-core consumers, now extended to support per-agent TDT config overrides. This delta supersedes the existing spec's `consumer:` section convention with the `runtime:` section and `~/.tdt/agents/` namespace.

## MODIFIED Requirements

### Requirement: ConsumerConfig composes Settings

`ConsumerConfig` SHALL compose agent-core's `Settings` as a field (not inheritance). The `Settings` instance SHALL reflect the merged global + agent-specific configuration from `~/.tdt/agents/{consumer-name}.yaml`.
