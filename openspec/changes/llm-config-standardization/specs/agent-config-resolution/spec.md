...
Agent-specific env var prefixes follow the convention: uppercase agent name with hyphens replaced by underscores, followed by an underscore.
...
The system SHALL provide a `tdt_agents_dir()` function in `tdt_core.paths` that returns the canonical `~/.tdt/agents/` directory path.
...
The model factory (`_ai/models.py`) MUST NOT read config files directly. It SHALL receive provider and model configuration as dict parameters from the caller. The functions `_load_tdt_model_config()` and `_load_tdt_providers()` SHALL be removed. API key resolution remains as an internal helper (`_resolve_api_key()`).
...
- **AND** API key resolution SHALL use `_resolve_api_key()` at call time
...
All agent consumers (agent-core, agent-docs-sync, agent-harness) SHALL use `load_agent_config()` to resolve model configuration instead of calling `load_settings()` for model-related config.
