...
- [x] 1.4 Run strict focused OpenSpec validation and inspect parsed deltas before mutation.

## 2. Live Configuration

- [x] 2.1 Create a timestamped local backup of `~/.hermes/config.yaml` and verify matching SHA-256 values.
- [x] 2.2 Set `providers.cockpit.model` to `gpt-5.6-luna` and atomically rewrite only the `moa` section: remove default's extra `Advance` reference; set all cockpit slots to `gpt-5.6-luna` at `max`; set default aggregator to cockpit Luna max; restore `privacy_filter: display`.
- [x] 2.3 Assert YAML shape: exact three presets, exact target slots, no cockpit `gpt-5.6-sol`, no stale typo, preserved token/temperature/cadence/degraded-policy settings, and provider/model context length 1M.
- [x] 2.4 Verify `hermes config check`, `hermes config get moa`, `hermes moa list`, and `hermes fallback list` after mutation.
...
- [x] 4.1 Run fresh direct inference checks after mutation for cockpit Luna, fable-5, and Advance.
