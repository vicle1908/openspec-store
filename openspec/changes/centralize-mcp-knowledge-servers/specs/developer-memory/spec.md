...
| DM-002 | developer-memory | Agentmemory is wired to Cursor | e2e | fixture client topology + MCP Router | `knowledge-topology-test --client cursor --expect-router-only`; router-mediated tools/list and no direct provider entry | artifacts/verification/cursor-router-memory.out |
| DM-003 | developer-memory | Agentmemory is wired to Claude Code | e2e | fixture client topology + MCP Router | `knowledge-topology-test --client claude --expect-router-only`; hooks preserved and router-mediated tools/list | artifacts/verification/claude-router-memory.out |
| DM-004 | developer-memory | Agentmemory is wired to Codex CLI | e2e | fixture client topology + MCP Router | `knowledge-topology-test --client codex --expect-router-only`; mirrored hooks preserved and router-mediated tools/list | artifacts/verification/codex-router-memory.out |
| DM-005 | developer-memory | Agentmemory is wired to OpenCode | e2e | fixture client topology + MCP Router | `knowledge-topology-test --client opencode --expect-router-only`; router-mediated tools/list and no direct provider entry | artifacts/verification/opencode-router-memory.out |
| DM-006 | developer-memory | Agentmemory is wired to pi | e2e | .pi/agent/extensions/agentmemory | `test -f ~/.pi/agent/extensions/agentmemory/index.js`; extension calls router boundary | artifacts/verification/pi-router-memory.out |
...
| DM-008 | developer-memory | Canonical engine is unavailable | integration | fail-closed boundary fixture | `agentmemory-boundary-test --engine-down --after-connect`; read/write return engine-unavailable and local KV remains unchanged | artifacts/verification/memory-engine-down.out |
| DM-009 | developer-memory | Cross-client shared recall is verified | e2e | two authenticated router clients | `agentmemory-boundary-test --two-clients --identity-proof --timeout-policy`; distinct server-derived audit attribution and shared engine identity | artifacts/verification/memory-cross-client.out |
| DM-010 | developer-memory | Memory test cleanup is verified | integration | tagged test observations | `agentmemory-test-cleanup --tag <generated-test-tag>`; deletion succeeds or blocks closure with retained failure evidence | artifacts/verification/memory-cleanup.out |

## Scenario/Test Traceability

Every modified or added scenario has a stable verification ID in the table above or in the feature matrix owned by `tasks.md` 2–4 and 7.2. A test is not considered complete without its command, expected result, redacted retained evidence path, and explicit negative-path assertion.

## MODIFIED Verification

The legacy direct-registration checks are retired. Readiness MUST verify the shared MCP Router boundary and the absence of direct AgentMemory entries; a direct `agentmemory` string in a client config is a failure unless an approved compatibility exception is active.

### Requirement: No Go service code is modified

The developer-memory integration SHALL NOT modify any Go service code, package structure, or dependency graph. The integration is entirely in developer tooling.
...
The exact supported-client matrix, including uninstalled clients and fixture formats, is authoritative in `tasks.md` and `review-scope.yaml`; live absence of a client is not evidence that its declared topology is unsupported.
