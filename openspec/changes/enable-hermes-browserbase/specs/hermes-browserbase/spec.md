# Hermes Browserbase Cloud Browsing
...
`browser.cloud_provider` SHALL select `browserbase` instead of `local`. This changes the default execution path for public native `browser_*` calls from local Chromium to Browserbase cloud sessions.

#### Scenario: Local browsing is preserved for private URLs

- Given `browser.cloud_provider` is `browserbase`
- And `browser.auto_local_for_private_urls` is `true`
- When native `browser_navigate` is called with a loopback URL
- Then the request SHALL use a local Chromium sidecar

## REMOVED Requirements

- None. No existing capability requirement is removed.
