...
- [ ] 1.3 Search all workspace repos (`~/Developer/*/`) and `~/.hermes/` for the canonical version-controlled source of the four custom skills. If none is found, select one (recommended: `openspec-store` as the canonical source with a `skills/` directory).
