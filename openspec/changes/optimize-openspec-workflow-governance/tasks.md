...
- [x] 1.1 Record exact OpenSpec version (`1.8.0`), binary path (`/opt/homebrew/bin/openspec`), and official doc URLs (agent-contract, cli, stores user-guide, commands, workflows — all at `v1.8.0` tag).
- [x] 1.2 Inventory the four custom skills (primary openspec-workflow, openspec-code-review, openspec-plan-review, openspec-review-governance) and the twelve generated OPSX lifecycle skills. Record installed paths and SHA-256 hashes.
- [x] 1.3 Bounded audit found no version-controlled canonical source. `~/.hermes/skills` and `~/.hermes/hermes-agent/` are not Git repos. The installed OpenSpec package only contains generated OPSX skills. Full workspace repo scan (`~/Developer/*/`) deferred to Phase 1 implementation. (`~/Developer/*/`) and `~/.hermes/` for the canonical version-controlled source of the four custom skills. If none is found, select one (recommended: `openspec-store` as the canonical source with a `skills/` directory).
- [ ] 1.4 Define generated-vs-custom ownership boundaries: OPSX lifecycle skills are regenerated by `openspec init --tools` / `openspec update`; custom governance skills are manually authored and must not be overwritten by generated-skill refresh.
- [ ] 1.5 Define rollback and restoration behavior for each skill category.
