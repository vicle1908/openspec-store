# mr-review-orchestration Specification

## Purpose

`mr-review-orchestration` is the capability that governs how the `ai-review` service resolves the file-change scope for a GitLab MR review. It is responsible for choosing the diff source (worktree-based local git vs GitLab API), managing worktree lifecycle, and surfacing observability events for every decision.
## Requirements

_(Baseline: no requirements defined. All requirements are introduced by the `ai-review-mr-scope-correctness` change.)_
