...
Define Android receiver-state detectors that identify registration and unregistration paths capable of diverging from their tracking flags.
...
### Requirement: L-RX detectors SHALL execute through the production scanner

The Android plugin's context-aware L-RX detectors SHALL be invoked by `GrepScanner.scan()` once per eligible existing non-test changed source file when the corresponding canonical rule ID is loaded. Detector execution SHALL NOT depend on a pre-existing raw L-RX match.

#### Scenario: Changed file has no raw L-RX match

- **WHEN** a changed Kotlin file satisfies an L-RX invariant and ripgrep produces no raw L-RX finding
- **THEN** the loaded context-aware detector SHALL inspect the full file and emit the finding through the scanner result

#### Scenario: Detector rule is not loaded

- **WHEN** the plugin exposes a detector but its canonical rule ID is absent from the loaded rules
- **THEN** the scanner SHALL NOT execute or emit that detector's finding

### Requirement: L-RX production integration SHALL preserve exclusions and uniqueness

Production scanner execution SHALL preserve canonical false-positive exclusions and SHALL deduplicate emitted findings by `(rule_id, file_path, line)`.

#### Scenario: Multiple raw findings refer to one file

- **WHEN** several raw scanner findings refer to the same eligible changed file
- **THEN** each loaded L-RX detector SHALL execute at most once for that file
- **AND** duplicate L-RX findings SHALL NOT be introduced
