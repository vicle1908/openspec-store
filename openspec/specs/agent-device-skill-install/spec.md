# agent-device-skill-install Specification

## Purpose
Defines the first-class install flow for the `agent-device` skill into the TDT skills root, operator-level CLI prerequisites, and the thin-router boundary between the installed skill and TDT-owned documentation.
