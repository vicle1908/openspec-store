# platform-kafka-harness Specification

## Purpose

This spec defines the Kafka consumer harness: configurable consumer group + topic, idempotent consumer pattern with aggregate-version gap detection, non-blocking retry topic pattern, Burrow-based consumer lag alerting, and crash-recovery semantics. The platform SHALL NOT use Kafka for synchronous RPC (HTTP / Temporal are the right tools for that).
...
> **Status**: IMPLEMENTED. `AggregateVersionStore` interface in `platform/kafka/receipt.go`, `PostgresAggregateVersionStore` with atomic CAS, `MemoryAggregateVersionStore` for tests, `Consumer.AggregateVersions` config field, migration in `platform/migrations/0001_kafka_aggregate_versions.sql`.
