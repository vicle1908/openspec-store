...
## Purpose
This spec defines the payment-service extraction. The payment-service SHALL own the `payment` Postgres schema as sole writer, publish payment events to the `payments.events.v1` Kafka topic via Debezium CDC, and run `PaymentCaptureWorkflow` and `PaymentRefundWorkflow` Temporal workflows on task queue `payment.capture.v1`. The service SHALL expose REST endpoints for capture (auto-create + capture in one shot) and refund; the order-worker SHALL call these endpoints as remote activities.
## Requirements
### Requirement: payment-service is an independently deployable Go module

> **Status**: IMPLEMENTED. payment-service exists as independent Go module with hexagonal layout.
...
> **Status**: LOCAL-VERIFIED. The canonical payment connector, exact
> publication and slot, idempotent Compose/kind registration, and a retained
> local acceptance probe prove delivery from `payment.payment_outbox` to
> `payments.events.v1`. This status does not claim cloud deployment readiness.
...
> **Status**: IMPLEMENTED. REST endpoints exist for intent creation, capture, refund, and status queries.
...
> **Status**: IMPLEMENTED. Operation ID derived from workflow ID using SHA-256 helper; idempotency enforced.
...
> **Status**: IMPLEMENTED. ADR exists at services/payment-service/docs/adr/0001-service-extraction.md.
