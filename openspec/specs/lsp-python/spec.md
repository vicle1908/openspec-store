...
OMP LSP config resolution SHALL be cwd-only. Root markers (e.g. `pyproject.toml`) MUST exist in the agent's working directory for basedpyright to activate. Project-level `.omp/lsp.json` SHALL be loaded only when the agent launches from within the project directory.
