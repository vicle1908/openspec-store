...
- **AND** canonical source or native provider unavailability SHALL NOT by itself make the model-free command fail
...
#### Scenario: LLM discovery option reaches a real boundary or is removed

- **GIVEN** the public CLI advertises `--use-llm` for discovery
- **WHEN** the option is selected
- **THEN** it SHALL reach an implemented model-backed discovery composition boundary using one captured canonical context
- **OR** the option SHALL be removed from parsing, help, documentation, and tests with actionable migration guidance
