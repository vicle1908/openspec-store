# epic-plan-extraction Specification

## Purpose
Extract structured data from a configured Epic Plan spreadsheet to provide planned development windows, sprint overlaps, target releases, and release gates for plan-aware epic analysis. Enriches Jira actuals with authoritative plan values without modifying workbook ownership or stakeholder tabs.
...
The system SHALL distinguish explicitly managed `jira-epic-report` output tabs from protected or unmanaged workbook tabs. The canonical list of managed output tabs is defined in `report-generation`'s "Canonical managed output tabs" table. Sheet synchronization MUST add, replace, or delete only managed output tabs from that list and MUST preserve `Epic Plan` and every unknown/unmanaged tab.
