# platform-projection Specification

## Purpose

Provides a reusable projection consumer framework using franz-go with cooperative balancing, atomic DB+cursor commits in a single PostgreSQL transaction, freshness lag reporting via OpenTelemetry histograms, and a reconcile CLI tool for stuck partitions.

## Requirements
### Requirement: Platform projection pattern is generic and reusable

> **Status**: IMPLEMENTED. Platform projection library exists in platform/projection with consumer, writer, freshness reporter.
...
> **Status**: IMPLEMENTED. Projection tables carry cursor columns for source-event tracking.
...
> **Status**: IMPLEMENTED. Projection writes atomic; row and cursor advance in same transaction.
...
> **Status**: IMPLEMENTED. Freshness metric published via OTel histogram on each projection write.
...
> **Status**: IMPLEMENTED. CLI cmd/projection-reconcile exists for stuck partition detection.
...
> **Status**: IMPLEMENTED. Projection consumer configured with MaxConcurrentFetches(8) and async commits.
