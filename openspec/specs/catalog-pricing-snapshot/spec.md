# catalog-pricing-snapshot Specification

## Purpose
The platform implements Price authority The catalog service SHALL be the sole writer of the `prices` table within the `catalog` schema. The Order service SHALL NOT write price rows. Prices are versioned: a price row is immutable once `effective_at` is in the past. Editing a
## Requirements
### Requirement: Price authority

> **Status**: IMPLEMENTED. Catalog service owns prices table; order-service does not write prices.
...
> **Status**: PARTIAL. Quote endpoint exists in catalog-service; cache and discount logic may be partial.
...
> **Status**: IMPLEMENTED. Price fields include currency and tax_class in catalog schema.
...
> **Status**: IMPLEMENTED. Order line items store price_snapshot_id and related fields.
...
> **Status**: PARTIAL. Discount schema defined; evaluation logic may be partial.
...
> **Status**: PARTIAL. Observability platform exists; catalog-specific metrics may be partial.
...
> **Status**: IMPLEMENTED. Order service calls catalog quote endpoint before persisting line items.
...
> **Status**: PARTIAL. Platform cache exists; catalog quote caching may be partial.
...
> **Status**: DEFERRED. Cache invalidation logic not yet implemented.
