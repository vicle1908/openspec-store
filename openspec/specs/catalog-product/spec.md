...
## Purpose
The platform implements Product aggregate The catalog service SHALL own a `Product` aggregate with identity `product_id` (ULID), fields `sku` (unique per environment), `display_name`, `description`, `status` (one of `draft`, `active`, `archived`), `category_id`, `attribute_
## Requirements
### Requirement: Product aggregate

> **Status**: IMPLEMENTED. Product aggregate exists in catalog-service/internal/domain with ULID identity and status lifecycle.
...
> **Status**: PARTIAL. Variant schema exists; add/update/remove operations may be partial.
...
> **Status**: IMPLEMENTED. REST endpoints exist in catalog-service/internal/adapters/http with pagination and idempotency.
...
> **Status**: PARTIAL. Schema validation exists; enforcement logic may be partial.
...
> **Status**: PARTIAL. Category schema exists; tree management and soft-delete may be partial.
...
> **Status**: LOCAL-VERIFIED. Catalog events use the transactional outbox, and
> the canonical connector plus idempotent Compose/kind registration and retained
> local acceptance prove `catalog.outbox` delivery to `catalog.events.v1`.
...
> **Status**: LOCAL-VERIFIED. The canonical catalog connector and registration
> path are statically aligned with the migration, and retained local acceptance
...
> **Status**: IMPLEMENTED. Synchronous REST for CRUD; Temporal used only for long-running operations.
