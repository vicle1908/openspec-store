# agent-device-verify-loop Specification

## Purpose
Establishes the canonical device verification loop (`open → snapshot → act → verify → close`), ref/selector semantics, session hygiene, and platform-specific interaction rules.
