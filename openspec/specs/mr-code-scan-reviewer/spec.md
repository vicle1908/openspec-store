...
Findings published from any review path SHALL pass through one shared gate that compares each finding's file path and line number against the MR diff snapshot's changed-line ranges. Findings on files or lines outside the snapshot SHALL be dropped with reason `file_not_in_diff` for an absent file or `line_not_in_diff` for a line outside the file's changed ranges.
...
- **THEN** the finding SHALL be dropped with reason `file_not_in_diff`
