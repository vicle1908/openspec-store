# notification-aggregate Specification

## Purpose
The platform implements Notification aggregate The notification service SHALL own a `Notification` aggregate whose identity is a ULID minted at creation, with fields `recipient_id`, `channel` (one of `email`, `sms`, `push`), `template_id`, `template_version`, `payload` (val
## Requirements
### Requirement: Notification aggregate

> **Status**: IMPLEMENTED. Notification aggregate exists in notification-service/internal/domain with ULID identity and status lifecycle.
...
> **Status**: IMPLEMENTED. Idempotency key requirement enforced; duplicate requests return original ID.
...
> **Status**: PARTIAL. Template versioning schema exists; dispatch-time loading may be partial.
...
> **Status**: IMPLEMENTED. Status transitions enforced with typed errors for invalid transitions.
...
> **Status**: IMPLEMENTED. Kafka consumption with cooperative-sticky balancing and inbox dedupe implemented.
