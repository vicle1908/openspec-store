...
This spec defines the protobuf contract pipeline, typed event registry, and contract-version policy. The platform SHALL enforce that every cross-service wire format has a versioned schema in `proto/`, a generated Go binding under `contracts/<service>/<v>/`, and a registry entry that downstream consumers consult at compile time.
...
### Requirement: protovalidate annotations on all input messages (PARTIAL)
> **Status**: `platform/contracts/validate.go` helper exists. Full proto annotation standardization across all proto files is in progress.
...
### Requirement: `buf.gen.yaml` uses version-pinned local plugins
Contract-owning services SHALL configure `buf.gen.yaml` with Buf's supported
`local:` plugin path and SHALL use repository-local, exact-version executables.
The current Go binding workflow pins Buf `1.71.0` and `protoc-gen-go`
`v1.36.10`. Generation SHALL run twice in isolated clean output directories,
prove byte-identical output, and compare it with checked-in bindings. Registry
authentication, quota, or network failures MUST NOT be treated as successful
generation.

#### Scenario: Local generation uses exact repository tools
- **WHEN** order, customer, or reporting runs `make generate-check`
- **THEN** the command verifies exact Buf and `protoc-gen-go` versions before linting, building, and generating
- **AND** a missing or mismatched executable fails with a tooling category

#### Scenario: Pinning versions means reproducible builds
- **WHEN** two services run `buf generate` from different developer machines
- **THEN** both produce byte-identical generated Go code because the generator executable and configuration are pinned

#### Scenario: Checked-in bindings drift
- **WHEN** clean pinned generation differs from the checked-in `contracts/` directory
- **THEN** `make generate-check` exits non-zero with `generated-drift` and retains machine-readable evidence
